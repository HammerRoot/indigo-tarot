...
- **无 HTTPS**（✅ 2026-09-16 已决策：接受现状）：公网 IP 直连只能用 HTTP，无法绑标准证书。除观感外有实质影响——用户自填的 DeepSeek Key、问题内容、`ADMIN_TOKEN` 均明文过网；如需 HTTPS，后续加域名 + ICP 备案 + SSL 证书（现有前置 Nginx 直接加 443 server 块即可，应用侧零改动）。**（443 黑洞曾让部分访客完全打不开，已由 N10 修复，2026-09-17）**
...
- **Redis / 服务器安全**：自建 Redis 虽只监听本机，仍需强密码；服务器放行 22 / 80 / **443**（**443 必须放行**——关掉会变回丢包黑洞、致部分访客彻底打不开，见台账 N10），建议 SSH 用密钥登录、`.env.local` 权限 600。
