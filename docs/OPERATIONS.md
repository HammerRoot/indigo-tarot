...
| 发布方式 | **人确认后手工触发**：服务器上 `./deploy.sh`（git 部署，决策 D16；无 CI/CD） |
...
>
> 表格只记**状态**（是什么、完成没有、谁确认）；下面「条目过程与理由」记**当时怎么做的**、**为什么这么做**。
>
...
| N8 | 补齐 Redis 分支测试（quota / rate-limit / trial，26 例） | ✅ 已完成（2026-09-16）| 我 |
| N9 | Redis 故障静默降级 → 已接入外部探测告警 | ✅ 已完成（2026-09-16） | 你 |
| N10 | 443 由丢包黑洞改为快速拒绝（避免访客彻底打不开） | ✅ 已完成（2026-09-17） | 你 |
| N11 | 部署方式由 tar 覆盖改为 git + `deploy.sh` | ✅ 已完成（2026-09-18） | 我 |
...
- **安全基线**：自建 Redis 只监听本机 + `requirepass` + AOF；服务器放行 22 / 80 / **443**
  （**443 必须放行**，见 N10）；`.env.local` 权限 600。
...
> **本节只列"已接受、无计划改动"的限制。** 有明确计划的（暂缓 / 远期）不在这里——
> 见 [`plan/`](./plan/) 目录（**目录内容即清单**）。
