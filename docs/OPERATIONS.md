...
| N10 | 443 是**丢包黑洞**，把"HTTPS 升级失败"放大成"访客彻底打不开" | ⬜ 待办 | 你 | **2026-09-17 发现**：Chrome 的 HTTPS Upgrades（`chrome://flags/#https-upgrades`）会把 `http://` 导航升级到 `https://`，本意是"升级失败就回落 HTTP"；但本机 443 被防火墙**丢包**（实测 `Connection timed out`，不是拒绝）——**"失败"变成了"等待"，回落逻辑永远等不到信号**，访客只看到超时页（`ERR_CONNECTION_TIMED_OUT`）。<br>**改法**：让 443 **立即拒绝**而非丢包。⚠️ 前提：若 443 是在腾讯云防火墙被丢的，包根本到不了服务器，须**先在云防火墙放行 443**，再在服务器侧 `iptables -A INPUT -p tcp --dport 443 -j REJECT --reject-with tcp-reset`。<br>**验证**：① `curl -sS --max-time 5 https://<SERVER_IP>/` 应**立刻**报拒绝（而非卡满 5 秒超时）；② 用一个全新 Chrome 配置档（无 HTTP 许可记忆）访问站点，应能**自动回落**到 HTTP、正常打开。<br>**收益不对称**：对现有访客零影响；若确是大范围问题，救回的是整个新访客群体 |
