...
| D16 | **部署改 git + `deploy.sh`**，不追求 CI/CD 自动部署 | 旧 tar 覆盖流程有三个衍生的复杂度（镜像 curl、对账残留、手工 `.deployed-sha` 锚点）全部源于 tar 本身；`git fetch + reset --hard` 自动删文件、git 本身即锚点。**上限是"人 ssh 上去跑一条命令"**——项目纪律要求发布终点是人确认（D1 随仓库提交、第 6 节发布纪律），自动部署等于跳过这一步 |
| D17 | **DEPLOYMENT.md 只写现状**，历史痕迹（事故经过/纠错记录）归 archive | docs/README.md 职责表已声明 DEPLOYMENT.md「禁止出现迁移历史、事故经过」，此前正文越界塞了 8 处「实测/初稿/发生」叙述。收敛原则：纯历史纠错**删**、事故**过程移 archive**、约束**保留结论**（如 X-Forwarded-For 必须覆写） |
