...
> **本文只负责三件事**：① 文档规则与地图；② 规格台账（条目状态 + 决策 D1–D11 + 测试规模）；
> ③ 文件清单（活跃 / 归档）。
...
| **本文** | 文档规则与地图、**规格条目状态**、**决策 D1–D11（唯一定义处）**、**测试规模（唯一定义处）** | 部署现状、运维待办 |
...
| **本文** | 文档规则 + 规格台账（条目状态 / 决策 / 测试规模）+ 文件清单 |
...
本项目采用**规格驱动开发（SDD）+ 测试驱动开发（TDD）**：所有修复与优化先在 `archive/` 的对应文档中
...
| G9 | 选牌改为平铺网格（移除扇形/旋转/缩放，减去已选牌） | 🟢 | 质量提升（交互） | ✅ 完成（网格移至 `/draw/select`；列数已由 G11 改一行 8 张） |
...
**当前无待开发条目。** 运维侧剩余待办见 [`OPERATIONS.md`](./OPERATIONS.md)（本文不重复）。

### 测试规模

**224 个测试 / 35 个文件**，`type-check` / `lint` / `test:run` 全绿。

> 复核方式：`npm run test:run`。**本文是测试规模的唯一定义处**，其他文档不得复写此数字，只能链接本文。
> 历史快照（如某次升级当天的测试数）不再记录具体数字，以免与当前值混淆。
