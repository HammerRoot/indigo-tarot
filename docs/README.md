...
| [`SPEC.md`](./SPEC.md) | **唯一的规格台账**：条目状态、质量门禁、**决策记录 D1–D18（唯一定义处）**。**随收口更新** | 文档规则、部署现状、运维待办 |
...
| 目录 | 唯一负责 | 规则 |
|---|---|---|
| [`plan/`](./plan/) | **在飞的规格**（一个功能点一个文件） | 开工前先在这里写清**验收标准 + 测试计划**；条目完成并归档后，才在 [`SPEC.md`](./SPEC.md) 补一行；作废 / 搁置**就地删除或改写**，不留在目录里冒充待办 |
