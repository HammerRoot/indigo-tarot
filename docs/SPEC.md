...
| D17 | **`DEPLOYMENT.md` 只写现状**：其历史痕迹（事故经过、纠错记录、实测过程）归 `archive/`。
**`OPERATIONS.md` 例外**——它**保留** N 系列各条的**过程与理由**（排查、实测数据、踩坑），与状态表同处 §二 | 两份文档的定位不同：`DEPLOYMENT.md` 是"照做即可"的操作手册，过程叙述会淹没步骤；
