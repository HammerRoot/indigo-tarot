...
| 编号 | 事项 | 现状 | 责任 | 说明 |
|---|---|---|---|---|
| N1 | 监控告警配置（UptimeRobot 关键字 + 腾讯云云监控） | 未配置 | **你**（平台账号操作） | 收益最高，见下方执行要点 |
...
- **无 HTTPS**：公网 IP 直连只能用 HTTP，无法绑标准证书。除观感外有实质影响——用户自填的 DeepSeek Key、问题内容、`ADMIN_TOKEN` 均明文过网；如需 HTTPS，后续加域名 + ICP 备案 + SSL 证书。
...
| 项 | 谁确认 | 如何确认 |
|---|---|---|
| 监控告警已配置（UptimeRobot 关键字 + 云监控无数据告警） | **你** | UptimeRobot / 腾讯云控制台截图或状态页 |
| Vercel 旧部署已删除或已移除系统 Key | 共同（我初查可达性 → 你控制台处置） | 我已可查域名是否可访问；删除/改配置需你操作 |
| 是否继续接受 HTTP 明文传输（安全 vs 免备案取舍） | **你**（决策） | 不做则维持现状，需在 README/文档中保留风险声明 |
...
| Redis 强口令、SSH 密钥登录 | **你**（服务器操作） | 服务器执行命令核对 |
