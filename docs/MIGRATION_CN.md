...
| 运行 | PM2（`indigo-tarot`，80 端口）+ Node 20.19.0，已配开机自启 |
...
| 编号 | 事项 | 现状 | 责任 | 说明 |
|---|---|---|---|---|
| N1 | 监控告警配置（UptimeRobot 关键字 + 腾讯云云监控） | ✅ 已完成（2026-09-16） | 你 | UptimeRobot 配了 Keyword + HTTP(s) 两条交叉验证；云监控阈值策略三条（CPU>80% / 内存>85% / 磁盘>85%，满足条件=任意）。**「无数据告警」入口未确认，暂缓**——UptimeRobot 已覆盖"服务器完全失联"场景 |
...
| N3 | 来源审计（调用日志 + 统计接口） | 待办（已确认保留） | 决策：你 / 实现：我 | 可选，需同步改 README 隐私表述 |
...
- 必配**「无数据」告警**：实例监控上报中断即机器失联，这是发现宕机的关键项。
...
| 编号 | 疑点 | 现状结论 | 待办 |
|---|---|---|---|
| Q1 | 客户端自带 `X-Forwarded-For` 是否可伪造限流键 | **已实测确认为「可伪造」**（2026-09-16）：带 `X-Forwarded-For: 203.0.113.77`（RFC 5737 测试网段）请求，Redis 中生成的键为 `rl:system_203.0.113.77`，而非请求的真实来源 IP → 换头即换限流桶。配合客户端可控的 `x-device-id`，**免费试用 + IP 限流可同时失效**。⚠️ 影响不止成本：每日 50 次配额是**全局计数器**，攻击者可一次烧光，使正常访客（含 HR/面试官）看到"额度已达上限"——从"烧钱问题"升级为"站点不可用问题" | **待决定加固方式**：<br>**A. 前置 Nginx 覆写 `X-Forwarded-For $remote_addr`**（正确做法，应用代码不用改，且为将来上 HTTPS 铺路；代价：新增组件 + 需把 Next 从 80 改到 3000 端口）<br>**B. 接受现状**（限流本就是尽力而为，配额仍是成本硬上限） |
...
| 项 | 谁确认 | 如何确认 |
|---|---|---|
| 监控告警已配置（UptimeRobot 关键字 + 云监控无数据告警） | ✅ 已完成（2026-09-16） | UptimeRobot 两条监控 + 云监控三条阈值策略；「无数据告警」未找到入口，暂缓 |
...
| 伪造 `X-Forwarded-For` 处置（Nginx 覆写或改用 socket 地址） | ⚠️ **待你决定**（Q1） | 已实测确认可伪造；方案 A 前置 Nginx / 方案 B 接受现状，见 Q1 行 |
